...
### `stages/surfaces/source_sink_graph.json`

Schema version: `source-sink-v1`

Generated by the `surfaces` stage. Traces network-facing endpoints through service components to exec sink binaries.

- `paths[]`: source→through→sink paths, each with `source`, `through` (component list), `sink`, and `confidence`
- `summary`: `total_paths`, `high_confidence`, `sources`, `sinks`

Consumers (e.g. threat_model, attack_surface) should reference high-confidence paths when building exploit hypothesis chains.

### `stages/findings/credential_mapping.json`

Schema version: `credential-mapping-v1`

Generated by the `findings` integrated step (`run_findings()`). Maps credentials found in firmware to auth surfaces.

- `mappings[]`: each entry includes `credential_type` (ssh_key, password_hash, api_token, default_credential), `file_path` (run-relative), `auth_surface` (ssh, web, os), `surface_component`, `confidence`, `risk_level` (high/medium/low)
- `summary`: `total_credentials`, `by_type`, `by_surface`, `high_risk`

All `file_path` values must be run-relative (absolute paths are rejected by the analyst report verifier).
