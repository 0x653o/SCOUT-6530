...
def write_analyst_report_v2_viewer(
    ...
    cred_path = report_dir.parent / "stages" / "findings" / "credential_mapping.json"
    if cred_path.is_file():
        try:
            cred_obj = cast(object, json.loads(cred_path.read_text(encoding="utf-8")))
            if isinstance(cred_obj, dict):
                cred_payload = cast(dict[str, JsonValue], cred_obj)
        except Exception:
            pass
    cred_bootstrap = json.dumps(
    ...
    cve_path = report_dir.parent / "stages" / "cve_scan" / "cve_matches.json"
    if cve_path.is_file():
        try:
            obj = cast(object, json.loads(cve_path.read_text(encoding="utf-8")))
            if isinstance(obj, dict):
    ...
    reach_path = report_dir.parent / "stages" / "reachability" / "reachability.json"
    if reach_path.is_file():
        try:
            obj = cast(object, json.loads(reach_path.read_text(encoding="utf-8")))
            if isinstance(obj, dict):
    ...
    for artifact_name, stage_subdir in [
        ...
        if apath.is_file():
            try:
                obj = cast(object, json.loads(apath.read_text(encoding="utf-8")))
                if isinstance(obj, dict):
    ...
    doc = "\n".join(
        [
            ...
            "",
            "    /* --- Pipeline bar --- */",
            "    .pipeline-bar { position: fixed; top: 56px; left: 0; right: 0; height: 2px; z-index: 99; display: flex; gap: 0; background: rgba(255,255,255,0.03); }",
            "    .pipeline-bar .seg { flex: 1; transition: background 0.3s; }",
            "    .pipeline-bar .seg.ok { background: var(--success); }",
            "    .pipeline-bar .seg.partial { background: var(--warning); }",
            "    .pipeline-bar .seg.failed { background: var(--danger); }",
            "    .pipeline-bar .seg.skipped { background: rgba(255,255,255,0.08); }",
            "    .pipeline-bar .seg.unknown { background: rgba(255,255,255,0.03); }",
            "",
            "    /* --- KPI summary bar --- */",
            "    .kpi-bar { position: fixed; top: 60px; left: 0; right: 0; height: 48px;",
            "      display: flex; align-items: center; justify-content: space-around;",
            "      background: var(--bg-secondary); border-bottom: 1px solid var(--line);",
            "      z-index: 98; padding: 0 2rem; }",
            "    .kpi-item { text-align: center; }",
            "    .kpi-value { font-size: 1.4rem; font-weight: 700; color: var(--accent); display: block; line-height: 1.2; }",
            "    .kpi-label { font-size: 0.65rem; text-transform: uppercase; letter-spacing: 0.08em; color: var(--muted); }",
            "    .kpi-danger { color: var(--danger); }",
            "    .kpi-warning { color: var(--warning); }",
            "",
            "    /* --- Page shell (centered flex layout) --- */",
            "    .page-shell { display: flex; max-width: 1800px; margin: 112px auto 0 auto; min-height: calc(100vh - 112px); padding: 0 32px; }",
            "",
            "    /* --- Sidebar --- */",
            "    .sidebar { position: sticky; top: 112px; width: 200px; flex-shrink: 0; height: calc(100vh - 112px); overflow-y: auto; padding: 16px 0;",
            "      background: rgba(255,255,255,0.03);",
            "      border-right: 1px solid rgba(255,255,255,0.06); }",
            "    .sidebar a { display: block; padding: 8px 16px; color: var(--ink-secondary); text-decoration: none; font-size: 0.75rem; font-weight: 500;",
            ...
            "    td { padding: 10px 14px; border-bottom: 1px solid rgba(255,255,255,0.03); color: var(--ink);",
            ...
            "    .candidate-table th { color: var(--ink-secondary); font-weight: 600; }",
            ...
            "    .ipc-table th { color: var(--ink-secondary); font-weight: 600; }",
            ...
            "      .kpi-bar { top: 56px; padding: 0 0.5rem; height: 40px; }",
            "      .kpi-value { font-size: 1rem; }",
            "      .kpi-label { font-size: 0.55rem; }",
            "      .sidebar { position: fixed; top: 100px; left: 0; bottom: 0; width: 260px; transform: translateX(-100%);",
            ...
            "  <!-- KPI Summary Bar -->",
            '  <div class="kpi-bar">',
            '    <div class="kpi-item"><span class="kpi-value" id="kpi-stages">--</span><span class="kpi-label">Stages</span></div>',
            '    <div class="kpi-item"><span class="kpi-value kpi-danger" id="kpi-critical">--</span><span class="kpi-label">Critical</span></div>',
            '    <div class="kpi-item"><span class="kpi-value kpi-warning" id="kpi-high">--</span><span class="kpi-label">High</span></div>',
            '    <div class="kpi-item"><span class="kpi-value" id="kpi-components">--</span><span class="kpi-label">Components</span></div>',
            '    <div class="kpi-item"><span class="kpi-value" id="kpi-cves">--</span><span class="kpi-label">CVEs</span></div>',
            '    <div class="kpi-item"><span class="kpi-value" id="kpi-endpoints">--</span><span class="kpi-label">Endpoints</span></div>',
            "  </div>",
            "",
            ...
            '    <a href="#pane-source-sink">Source → Sink</a>',
            '    <a href="#pane-credential-map">Credentials</a>',
            '    <a href="#pane-sbom">SBOM</a>',
            '    <a href="#pane-cve">CVE Scan</a>',
            '    <a href="#pane-reachability">Reachability</a>',
            '    <a href="#pane-secassess">Security Assessment</a>',
            ...
            "    </section>",
            "",
            '    <section class="card" id="pane-source-sink">',
            "      <h2>Source → Sink Paths</h2>",
            '      <div class="card-body" id="source-sink-vis"></div>',
            "    </section>",
            "",
            '    <section class="card" id="pane-credential-map">',
            "      <h2>Credential Map</h2>",
            '      <div class="card-body" id="credential-map"></div>',
            "    </section>",
            "",
            '    <section class="card" id="pane-sbom">',
            "      <h2>SBOM</h2>",
            '      <div class="card-body" id="sbom-content"></div>',
            "    </section>",
            "",
            '    <section class="card" id="pane-cve">',
            "      <h2>CVE Scan</h2>",
            '      <div class="card-body" id="cve-content"></div>',
            "    </section>",
            "",
            '    <section class="card" id="pane-reachability">',
            "      <h2>Reachability</h2>",
            '      <div class="card-body" id="reachability-content"></div>',
            "    </section>",
            "",
            '    <section class="card" id="pane-secassess">',
            "      <h2>Security Assessment</h2>",
            '      <div class="card-body" id="secassess-content"></div>',
            "    </section>",
            "",
            ...
            f"  {ipc_bootstrap}",
            "  </script>",
            '  <script id="bootstrap-source-sink-data" type="application/json">',
            f"  {ss_bootstrap}",
            "  </script>",
            '  <script id="bootstrap-credential-data" type="application/json">',
            f"  {cred_bootstrap}",
            "  </script>",
            '  <script id="bootstrap-sbom-data" type="application/json">',
            f"  {sbom_bootstrap}",
            "  </script>",
            '  <script id="bootstrap-cve-data" type="application/json">',
            f"  {cve_bootstrap}",
            "  </script>",
            '  <script id="bootstrap-reach-data" type="application/json">',
            f"  {reach_bootstrap}",
            "  </script>",
            '  <script id="bootstrap-secassess-data" type="application/json">',
            f"  {secassess_bootstrap}",
            "  </script>",
            "",
            "  <script>",
            "  /* ===== Utilities ===== */",
            ...
            "      window.scrollTo(0, 0);",
            "      /* Re-render graph when its pane becomes visible */",
            "      if (targetId === 'pane-graph' && typeof _lastGraphData !== 'undefined' && _lastGraphData) {",
            "        setTimeout(function() { renderGraph(_lastGraphData); }, 50);",
            "      }",
            "    }",
            "    document.querySelectorAll('.sidebar a').forEach(function(a) {",
            "      a.classList.remove('active');",
            "    });",
            "    var activeLink = document.querySelector('.sidebar a[href=\"#' + targetId + '\"]');",
            "    if (activeLink) activeLink.classList.add('active');",
            "    document.getElementById('sidebar').classList.remove('open');",
            "  }",
            "",
            "  /* ===== Global search (single-pane aware) ===== */",
            ...
            "        if (href) showPane(href.substring(1));",
            "      }",
            "      document.querySelectorAll('.sidebar a').forEach(function(a) { a.style.display = ''; });",
            "      return;",
            "    }",
            "    document.querySelectorAll('.card[id^=\"pane-\"]').forEach(function(card) {",
            "      var text = (card.textContent || '').toLowerCase();",
            "      card.style.display = text.indexOf(q) >= 0 ? 'block' : 'none';",
            "    });",
            "    document.querySelectorAll('.sidebar a[href^=\"#pane-\"]').forEach(function(a) {",
            "      var paneId = a.getAttribute('href').substring(1);",
            "      var pane = document.getElementById(paneId);",
            "      a.style.display = (pane && pane.style.display !== 'none') ? '' : 'none';",
            "    });",
            "  }",
            ...
            "  });",
            "  document.querySelectorAll('.sidebar a[href^=\"#pane-\"]').forEach(function(link) {",
            ...
            "  var firstNavLink = document.querySelector('.sidebar a[href^=\"#pane-\"]');",
            "  if (firstNavLink) { showPane(firstNavLink.getAttribute('href').substring(1)); }",
            "",
            "  /* ===== Evidence modal ===== */",
            ...
            "    var binaries = (ipcData && typeof ipcData === 'object' && Array.isArray(ipcData.hits)) ? ipcData.hits : [];",
            ...
            "    var binaries = Array.isArray(data.hits) ? data.hits : [];",
            ...
            "    try { var d2 = JSON.parse(bn.textContent || '{}'); return (d2 && typeof d2 === 'object') ? d2 : {}; } catch(_) { return {}; }",
            "  }",
            "  async function loadSourceSink() {",
            "    try { var r = await fetch('../stages/surfaces/source_sink_graph.json', { cache: 'no-store' }); if (r.ok) { try { var d = await r.json(); return (d && typeof d === 'object') ? d : {}; } catch(_) {} } } catch(_) {}",
            "    var bn = document.getElementById('bootstrap-source-sink-data'); if (!bn) return {};",
            "    try { var d2 = JSON.parse(bn.textContent || '{}'); return (d2 && typeof d2 === 'object') ? d2 : {}; } catch(_) { return {}; }",
            "  }",
            "  async function loadCredentials() {",
            "    try { var r = await fetch('../stages/findings/credential_mapping.json', { cache: 'no-store' }); if (r.ok) { try { var d = await r.json(); return (d && typeof d === 'object') ? d : {}; } catch(_) {} } } catch(_) {}",
            "    var bn = document.getElementById('bootstrap-credential-data'); if (!bn) return {};",
            "    try { var d2 = JSON.parse(bn.textContent || '{}'); return (d2 && typeof d2 === 'object') ? d2 : {}; } catch(_) { return {}; }",
            "  }",
            "  async function loadSbom() {",
            "    try { var r = await fetch('../stages/sbom/sbom.json', { cache: 'no-store' }); if (r.ok) { try { var d = await r.json(); return (d && typeof d === 'object') ? d : {}; } catch(_) {} } } catch(_) {}",
            "    var bn = document.getElementById('bootstrap-sbom-data'); if (!bn) return {};",
            "    try { var d2 = JSON.parse(bn.textContent || '{}'); return (d2 && typeof d2 === 'object') ? d2 : {}; } catch(_) { return {}; }",
            "  }",
            "  async function loadCve() {",
            "    try { var r = await fetch('../stages/cve_scan/cve_matches.json', { cache: 'no-store' }); if (r.ok) { try { var d = await r.json(); return (d && typeof d === 'object') ? d : {}; } catch(_) {} } } catch(_) {}",
            "    var bn = document.getElementById('bootstrap-cve-data'); if (!bn) return {};",
            "    try { var d2 = JSON.parse(bn.textContent || '{}'); return (d2 && typeof d2 === 'object') ? d2 : {}; } catch(_) { return {}; }",
            "  }",
            "  async function loadReach() {",
            "    try { var r = await fetch('../stages/reachability/reachability.json', { cache: 'no-store' }); if (r.ok) { try { var d = await r.json(); return (d && typeof d === 'object') ? d : {}; } catch(_) {} } } catch(_) {}",
            "    var bn = document.getElementById('bootstrap-reach-data'); if (!bn) return {};",
            "    try { var d2 = JSON.parse(bn.textContent || '{}'); return (d2 && typeof d2 === 'object') ? d2 : {}; } catch(_) { return {}; }",
            "  }",
            "  async function loadSecAssess() {",
            "    var result = {};",
            "    for (var item of [['cert_analysis','inventory'],['init_services','inventory'],['fs_permissions','inventory']]) {",
            "      try { var r = await fetch('../stages/' + item[1] + '/' + item[0] + '.json', { cache: 'no-store' }); if (r.ok) { try { var d = await r.json(); if (d && typeof d === 'object') result[item[0]] = d; } catch(_) {} } } catch(_) {}",
            "    }",
            "    if (Object.keys(result).length > 0) return result;",
            "    var bn = document.getElementById('bootstrap-secassess-data'); if (!bn) return {};",
            "    try { var d2 = JSON.parse(bn.textContent || '{}'); return (d2 && typeof d2 === 'object') ? d2 : {}; } catch(_) { return {}; }",
            "  }",
            "",
            "  /* ===== Source-Sink render ===== */",
            "  function renderSourceSink(ssData) {",
            "    var mount = document.getElementById('source-sink-vis');",
            "    if (!mount) return;",
            "    mount.innerHTML = '';",
            "    var data = (ssData && typeof ssData === 'object') ? ssData : {};",
            "    var paths = Array.isArray(data.paths) ? data.paths : [];",
            "    var summary = (data.summary && typeof data.summary === 'object') ? data.summary : {};",
            "    if (paths.length === 0) { mount.innerHTML = '<p class=\"muted\">No source\\u2192sink paths detected.</p>'; return; }",
            "    var grid = document.createElement('div'); grid.className = 'stat-grid';",
            "    [{l:'Total Paths', v: summary.total_paths || paths.length},",
            "     {l:'High Confidence', v: summary.high_confidence || 0},",
            "     {l:'Sources', v: summary.sources || 0},",
            "     {l:'Sinks', v: summary.sinks || 0}].forEach(function(s) {",
            "      var c = document.createElement('div'); c.className = 'stat-card';",
            "      var val = document.createElement('div'); val.className = 'stat-value';",
            "      val.textContent = String(s.v);",
            "      if (s.l === 'High Confidence' && s.v > 0) val.style.color = '#f87171';",
            "      var lbl = document.createElement('div'); lbl.className = 'stat-label'; lbl.textContent = s.l;",
            "      c.appendChild(val); c.appendChild(lbl); grid.appendChild(c);",
            "    });",
            "    mount.appendChild(grid);",
            "    paths.slice(0, 30).forEach(function(p) {",
            "      if (!p || typeof p !== 'object') return;",
            "      var card = document.createElement('div'); card.className = 'risk';",
            "      var src = (p.source && typeof p.source === 'object') ? p.source : {};",
            "      var sink = (p.sink && typeof p.sink === 'object') ? p.sink : {};",
            "      var through = Array.isArray(p.through) ? p.through : [];",
            "      var conf = Number(p.confidence) || 0;",
            "      var hdr = document.createElement('div'); hdr.style.cssText = 'display:flex;align-items:center;gap:8px;margin-bottom:8px';",
            "      var confBadge = document.createElement('span');",
            "      confBadge.className = 'badge ' + (conf >= 0.7 ? 'fail' : conf >= 0.4 ? 'blocked' : 'unknown');",
            "      confBadge.textContent = conf.toFixed(2);",
            "      hdr.appendChild(confBadge); card.appendChild(hdr);",
            "      var pathDiv = document.createElement('div'); pathDiv.style.cssText = 'display:flex;align-items:center;gap:6px;flex-wrap:wrap;font-size:0.82rem;margin-bottom:6px';",
            "      var srcSpan = document.createElement('span'); srcSpan.style.cssText = 'color:var(--accent);font-weight:600';",
            "      srcSpan.textContent = (src.type || '?') + ':' + (src.value || '?').split('/').pop();",
            "      pathDiv.appendChild(srcSpan);",
            "      pathDiv.appendChild(document.createTextNode(' \\u2192 '));",
            "      through.forEach(function(t, i) {",
            "        if (i > 0) pathDiv.appendChild(document.createTextNode(' \\u2192 '));",
            "        var tSpan = document.createElement('span'); tSpan.style.cssText = 'color:var(--ink-secondary)';",
            "        tSpan.textContent = (t && t.name) || '?';",
            "        pathDiv.appendChild(tSpan);",
            "      });",
            "      pathDiv.appendChild(document.createTextNode(' \\u2192 '));",
            "      var sinkSpan = document.createElement('span'); sinkSpan.style.cssText = 'color:var(--danger);font-weight:600';",
            "      sinkSpan.textContent = (sink.binary || '?') + '(' + ((sink.symbols || [])[0] || '?') + ')';",
            "      pathDiv.appendChild(sinkSpan);",
            "      card.appendChild(pathDiv);",
            "      mount.appendChild(card);",
            "    });",
            "  }",
            "",
            "  /* ===== Credential Map render ===== */",
            "  function renderCredentialMap(credData) {",
            "    var mount = document.getElementById('credential-map');",
            "    if (!mount) return;",
            "    mount.innerHTML = '';",
            "    var data = (credData && typeof credData === 'object') ? credData : {};",
            "    var mappings = Array.isArray(data.mappings) ? data.mappings : [];",
            "    var summary = (data.summary && typeof data.summary === 'object') ? data.summary : {};",
            "    if (mappings.length === 0) { mount.innerHTML = '<p class=\"muted\">No credential artifacts detected.</p>'; return; }",
            "    var grid = document.createElement('div'); grid.className = 'stat-grid';",
            "    [{l:'Total Credentials', v: summary.total_credentials || mappings.length},",
            "     {l:'High Risk', v: summary.high_risk || 0, c: 'var(--danger)'},",
            "     {l:'SSH', v: (summary.by_surface || {}).ssh || 0},",
            "     {l:'Web', v: (summary.by_surface || {}).web || 0},",
            "     {l:'OS', v: (summary.by_surface || {}).os || 0}].forEach(function(s) {",
            "      var c = document.createElement('div'); c.className = 'stat-card';",
            "      var val = document.createElement('div'); val.className = 'stat-value';",
            "      val.textContent = String(s.v);",
            "      if (s.c) val.style.color = s.c;",
            "      else if (s.v > 0) val.style.color = '#fbbf24';",
            "      var lbl = document.createElement('div'); lbl.className = 'stat-label'; lbl.textContent = s.l;",
            "      c.appendChild(val); c.appendChild(lbl); grid.appendChild(c);",
            "    });",
            "    mount.appendChild(grid);",
            ...
            "    var tbl = document.createElement('table');",
            "    var th = document.createElement('thead'); var hr = document.createElement('tr');",
            "    ['Risk', 'Type', 'File', 'Surface', 'Component'].forEach(function(l) {",
            "      var t = document.createElement('th'); t.textContent = l; hr.appendChild(t);",
            "    });",
            "    th.appendChild(hr); tbl.appendChild(th);",
            "    var tb = document.createElement('tbody');",
            ...
            "    });",
            "    sorted.slice(0, 50).forEach(function(m) {",
            "      var tr = document.createElement('tr');",
            "      var td1 = document.createElement('td');",
            "      var rb = document.createElement('span');",
            "      rb.className = 'badge ' + (m.risk_level === 'high' ? 'fail' : m.risk_level === 'medium' ? 'blocked' : 'unknown');",
            "      rb.textContent = m.risk_level || '?'; td1.appendChild(rb); tr.appendChild(td1);",
            "      var td2 = document.createElement('td'); td2.textContent = m.credential_type || '?'; tr.appendChild(td2);",
            "      var td3 = document.createElement('td'); td3.style.cssText = 'color:var(--accent);word-break:break-all;font-size:0.78rem';",
            "      td3.textContent = (m.file_path || '?').split('/').slice(-2).join('/'); tr.appendChild(td3);",
            "      var td4 = document.createElement('td');",
            "      var sb = document.createElement('span'); sb.className = 'badge'; sb.textContent = m.auth_surface || '?'; td4.appendChild(sb); tr.appendChild(td4);",
            "      var td5 = document.createElement('td'); td5.textContent = m.surface_component || '-'; tr.appendChild(td5);",
            "      tb.appendChild(tr);",
            "    });",
            "    tbl.appendChild(tb); wrap.appendChild(tbl); mount.appendChild(wrap);",
            "  }",
            ...
            "    var pane = document.getElementById('cve-content');",
            "    if (!pane || !data || !data.matches) return;",
            "    var matches = data.matches || [];",
            "    var h = document.createElement('h3'); h.textContent = 'CVE Matches (' + matches.length + ')'; pane.appendChild(h);",
            "    var s = data.summary || {};",
            "    var stats = document.createElement('div'); stats.className = 'stat-grid';",
            "    ['critical','high','medium','low'].forEach(function(sev) {",
            "      var card = document.createElement('div'); card.className = 'stat-card';",
            "      var label = document.createElement('div'); label.className = 'stat-label'; label.textContent = sev.toUpperCase();",
            "      var val = document.createElement('div'); val.className = 'stat-value'; val.textContent = String(s[sev]||0);",
            "      if (sev === 'critical') val.style.color = 'var(--danger)';",
            "      else if (sev === 'high') val.style.color = 'var(--warning)';",
            ...
            "    var pane = document.getElementById('reachability-content');",
            "    if (!pane || !data || !data.results) return;",
            "    var results = data.results || [];",
            "    var summary = data.summary || {};",
            "    var h = document.createElement('h3'); h.textContent = 'CVE Reachability Analysis'; pane.appendChild(h);",
            "    var stats = document.createElement('div'); stats.className = 'stat-grid';",
            "    [['Directly Reachable', summary.directly_reachable||0, 'var(--danger)'],",
            "     ['Potentially Reachable', summary.potentially_reachable||0, 'var(--warning)'],",
            "     ['Unreachable', summary.unreachable||0, 'var(--success)']].forEach(function(item) {",
            "      var card = document.createElement('div'); card.className = 'stat-card';",
            "      var label = document.createElement('div'); label.className = 'stat-label'; label.textContent = item[0];",
            "      var val = document.createElement('div'); val.className = 'stat-value'; val.textContent = String(item[1]); val.style.color = item[2];",
            "      card.appendChild(label); card.appendChild(val); stats.appendChild(card);",
            "    });",
            "    pane.appendChild(stats);",
            "    var tbl = document.createElement('table'); tbl.className = 'data-table';",
            "    var thead = document.createElement('tr');",
            "    ['Component','Version','Reachability','Hops','Path'].forEach(function(t){var th=document.createElement('th');th.textContent=t;thead.appendChild(th);}); tbl.appendChild(thead);",
            "    results.forEach(function(r) {",
            "      var tr = document.createElement('tr');",
            "      [r.component||'', r.version||''].forEach(function(v) { var td=document.createElement('td'); td.textContent=v; tr.appendChild(td); });",
            "      var reachTd = document.createElement('td'); reachTd.textContent = r.reachability||'';",
            "      reachTd.className = r.reachability==='directly_reachable'?'sev-critical':r.reachability==='potentially_reachable'?'sev-high':'sev-low';",
            "      tr.appendChild(reachTd);",
            "      var hopTd = document.createElement('td'); hopTd.textContent = String(r.hop_count||'-'); tr.appendChild(hopTd);",
            "      var pathTd = document.createElement('td'); pathTd.textContent = (r.path||[]).join(' \\u2192 '); pathTd.style.fontSize='0.7rem'; tr.appendChild(pathTd);",
            "      tbl.appendChild(tr);",
            "    });",
            ...
            "",
            "  /* ===== renderSecAssess ===== */",
            "  function renderSecAssess(data) {",
            "    var pane = document.getElementById('secassess-content');",
            "    if (!pane || !data) return;",
            "    var h = document.createElement('h3'); h.textContent = 'Security Assessment'; pane.appendChild(h);",
            "    var cert = data.cert_analysis;",
            "    if (cert && cert.issues) {",
            "      var ch = document.createElement('h4'); ch.textContent = 'Certificate Issues (' + cert.issues.length + ')'; pane.appendChild(ch);",
            "      var s = cert.summary || {};",
            "      var info = document.createElement('div'); info.className = 'stat-grid';",
            "      [['Expired', s.expired||0], ['Weak Key', s.weak_key||0], ['Weak Sig', s.weak_sig||0], ['Self-Signed', s.self_signed||0], ['Private Keys', s.private_keys_exposed||0]].forEach(function(item) {",
            "        var card = document.createElement('div'); card.className = 'stat-card';",
            "        var label = document.createElement('div'); label.className = 'stat-label'; label.textContent = item[0];",
            "        var val = document.createElement('div'); val.className = 'stat-value'; val.textContent = String(item[1]);",
            "        card.appendChild(label); card.appendChild(val); info.appendChild(card);",
            "      });",
            "      pane.appendChild(info);",
            "    }",
            "    var init = data.init_services;",
            "    if (init && init.issues) {",
            "      var ih = document.createElement('h4'); ih.textContent = 'Boot Service Risks (' + init.issues.length + ')'; pane.appendChild(ih);",
            "      init.issues.slice(0,20).forEach(function(issue) {",
            "        var d = document.createElement('div'); d.className = 'finding-card';",
            "        var badge = document.createElement('span'); badge.className = 'sev-badge sev-' + (issue.severity||'low'); badge.textContent = issue.severity||'';",
            "        var txt = document.createTextNode(' ' + (issue.service||issue.type||'') + ' \\u2014 ' + ((issue.details||{}).risk_reason||''));",
            "        d.appendChild(badge); d.appendChild(txt); pane.appendChild(d);",
            "      });",
            "    }",
            "    var fs = data.fs_permissions;",
            "    if (fs && fs.issues) {",
            "      var fh = document.createElement('h4'); fh.textContent = 'Permission Issues (' + fs.issues.length + ')'; pane.appendChild(fh);",
            "      var fsummary = fs.summary || {};",
            "      var fi = document.createElement('div'); fi.textContent = 'High: ' + (fsummary.high||0) + ' | Medium: ' + (fsummary.medium||0) + ' | Low: ' + (fsummary.low||0);",
            "      fi.style.color = 'var(--muted)'; fi.style.marginBottom = '0.5rem';",
            "      pane.appendChild(fi);",
            "    }",
            "  }",
            "",
            "  /* ===== KPI bar updater ===== */",
            "  function updateKpis(overview, sbomData, cveData) {",
            "    function setText(id, val) { var el = document.getElementById(id); if (el) el.textContent = String(val); }",
            "    var ov = (overview && typeof overview === 'object') ? overview : {};",
            "    var stages = ov.stages;",
            "    var stageCount = (stages && typeof stages === 'object') ? Object.keys(stages).length : 0;",
            "    var okCount = 0;",
            "    if (stages && typeof stages === 'object') {",
            "      Object.keys(stages).forEach(function(k) {",
            "        var s = stages[k]; if (s && typeof s === 'object' && s.status === 'ok') okCount++;",
            "      });",
            "    }",
            "    setText('kpi-stages', okCount + '/' + stageCount);",
            "    var sbom = (sbomData && typeof sbomData === 'object') ? sbomData : {};",
            "    var comps = Array.isArray(sbom.components) ? sbom.components.length : 0;",
            "    setText('kpi-components', comps);",
            "    var cve = (cveData && typeof cveData === 'object') ? cveData : {};",
            "    var cveSummary = (cve.summary && typeof cve.summary === 'object') ? cve.summary : {};",
            "    setText('kpi-critical', cveSummary.critical || 0);",
            "    setText('kpi-high', cveSummary.high || 0);",
            "    setText('kpi-cves', cve.total_cve_matches || 0);",
            "    var cockpit = (ov.cockpit && typeof ov.cockpit === 'object') ? ov.cockpit : {};",
            "    var ass = (cockpit.attack_surface_scale && typeof cockpit.attack_surface_scale === 'object') ? cockpit.attack_surface_scale : {};",
            "    var assData = (ass.data && typeof ass.data === 'object') ? ass.data : {};",
            "    setText('kpi-endpoints', assData.endpoints || 0);",
            "  }",
            "",
            "  /* ===== Render All Panes ===== */",
            "  function renderAllPanes(overview, digest, data, exploitCandidates, graphData, ipcData, ssData, credData, sbomData, cveData, reachData, secassessData) {",
            ...
            "    var safeSS = (ssData && typeof ssData === 'object') ? ssData : {};",
            "    var safeCred = (credData && typeof credData === 'object') ? credData : {};",
            "    var safeSbom = (sbomData && typeof sbomData === 'object') ? sbomData : {};",
            "    var safeCve = (cveData && typeof cveData === 'object') ? cveData : {};",
            "    var safeReach = (reachData && typeof reachData === 'object') ? reachData : {};",
            "    var safeSecAssess = (secassessData && typeof secassessData === 'object') ? secassessData : {};",
            ...
            "    safeRender(function() { renderSourceSink(safeSS); });",
            "    safeRender(function() { renderCredentialMap(safeCred); });",
            "    safeRender(function() { renderSbom(safeSbom); });",
            "    safeRender(function() { renderCve(safeCve); });",
            "    safeRender(function() { renderReach(safeReach); });",
            "    safeRender(function() { renderSecAssess(safeSecAssess); });",
            ...
            "    safeRender(function() { updateKpis(safeOverview, safeSbom, safeCve); });",
            "  }",
            "",
            "  /* ===== Main dispatch ===== */",
            "  // legacy: loadData().then(render)",
            "  // legacy: render({});",
            "  Promise.all([loadData(), loadOverview(), loadDigest(), loadExploitCandidates(), loadGraph(), loadIPC(), loadSourceSink(), loadCredentials(), loadSbom(), loadCve(), loadReach(), loadSecAssess()])",
            "    .then(function(results) {",
            "      renderAllPanes(results[1], results[2], results[0], results[3], results[4], results[5], results[6], results[7], results[8], results[9], results[10], results[11]);",
            "    }).catch(() => {",
            "      renderAllPanes({}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {});",
            "    });",
            "  </script>",
